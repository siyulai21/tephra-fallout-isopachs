"""
This is the backup of lsf_bsplines2d, without class implemented.
"""
...
def domain_divisions(xd, yd, fd):
    ...
    X = np.c_[np.ones_like(xd), xd, yd]
    beta, *_ = np.linalg.lstsq(X, fd, rcond=None)
    resid = fd - X.dot(beta)
...
def _assemble_coarse(xmin, ymin, xmax, ymax, mx, my, xd, yd, fd, sd, unl, und, rou, tau):
    neq = (mx + 3) * (my + 3)
    nband = mx * 3 + 13
    hx = (xmax - xmin) / mx
    hy = (ymax - ymin) / my
    ...
    bx = np.zeros(4); by = np.zeros(4)
    s10 = np.zeros((4,4)); s11 = np.zeros((4,4)); s12 = np.zeros((4,4))
    for k in range(len(xd)):
        x = float(xd[k]); y = float(yd[k])
        if (x < xmin) or (x > xmax) or (y < ymin) or (y > ymax): continue
        ip = int(np.floor((x - xmin) / hx)) + 1
        jp = int(np.floor((y - ymin) / hy)) + 1
        ...
        s = float(sd[k]); wd = 1.0 if s <= 0.0 else 1.0/(s*s)
        BSP3(rx, hx, 0, bx); BSP3(ry, hy, 0, by)
        for j1 in range(0,4):
            for i1 in range(0,4):
                ij1 = (ip+i1) + (mx+3)*(jp+j1-1)
                b[ij1-1] += bx[3-i1]*by[3-j1]*float(fd[k])*wd
                for j2 in range(j1,4):
                    for i2 in range(i1,4):
                        ij2 = (ip+i2) + (mx+3)*(jp+j2-1)
                        ij2b = ij2 - ij1 + 1
                        A[(ij1-1)*nband + (ij2b-1)] += bx[3-i1]*by[3-j1]*bx[3-i2]*by[3-j2]*wd
    SHFTI(0, s10); SHFTI(1, s11); SHFTI(2, s12)
    w1 = tau/rou * (unl/und)**2
    w2 = (1.0-tau)/rou * (unl**2/und)**2
    wx  = (hy/hx)*(w1/(unl**2))
    wy  = (hx/hy)*(w1/(unl**2))
    wkx = (hy/(hx**3))*(w2/(unl**2))
    wxy = (1.0/(hx*hy))*(w2/(unl**2))*2.0
    wyy = (hx/(hy**3))*(w2/(unl**2))
                ...
                for is_ in range(-3,3+1):
                    ...
                    if 1<=ij2b<=nband:
                        A[(ij1-1)*nband+(ij2b-1)] += (
                            wx*(s1x*s0y) + wy*(s0x*s1y) + wkx*(s2x*s0y) + wxy*(s1x*s1y) + wyy*(s0x*s2y)
                        )
    return A, b, nband, (mx+3)*(my+3)


def _prolongate(mx,my,cij):
    nx_f, ny_f = mx + 3, my + 3
    nx_c, ny_c = (mx // 2) + 3, (my // 2) + 3
    def Cget2D(i, j, kc): return cij[(j - 1) * kc + (i - 1)]
...
def _assemble_fine(xmin,ymin,xmax,ymax, mx,my, xd,yd, fd,sd, unl,und, rou,tau):
    nx, ny = mx + 3, my + 3
    hx = (xmax - xmin) / mx; hy = (ymax - ymin) / my
    a = np.zeros(16 * nx * ny); aint = np.zeros((4,4)); indp = np.zeros(nx*ny, dtype=int); b = np.zeros(nx*ny)
    bx = np.zeros(4); by = np.zeros(4)
    s10 = np.zeros((4,4)); s11 = np.zeros((4,4)); s12 = np.zeros((4,4))
    SHFTI(0, s10); SHFTI(1, s11); SHFTI(2, s12)
    w1 = tau/rou * (unl/und)**2
    ...
    for k in range(len(xd)):
        ...
        for j1 in range(0,4):
            for i1 in range(0,4):
                ...
                for j2 in range(j1,4):
                    for i2 in range(i1,4):
                        ib = i2 - i1 + 1; jb = j2 - j1 + 1
                        a[_AD_offset(ib, jb, ip1, jp1, nx)] += bx[3-i1]*by[3-j1]*bx[3-i2]*by[3-j2]*wd
        ...
        for i1 in range(1, nx+1):
            for js in range(0,4):
                for is_ in range(0,4):
                    ...
                    lx1 = max(1, is_+1, 5-i1); lx2 = min(4, is_+4, nx - i1 + 4)
                    s0x = s1x = s2x = 0.0
                    for lx in range(lx1, lx2+1):
                        s0x += s10[lx-1,(lx-is_)-1]
                        s1x += s11[lx-1,(lx-is_)-1]
                        s2x += s12[lx-1,(lx-is_)-1]
                    ly1 = max(1, js+1, 5-j1); ly2 = min(4, js+4, ny - j1 + 4)
                    s0y = s1y = s2y = 0.0
                    for ly in range(ly1, ly2+1):
                        s0y += s10[ly-1,(ly-js)-1]
                        s1y += s11[ly-1,(ly-js)-1]
                        s2y += s12[ly-1,(ly-js)-1]
                    val = (wx*(s1x*s0y) + wy*(s0x*s1y) + wkx*(s2x*s0y) + wxy*(s1x*s1y) + wyy*(s0x*s2y))
    ...
    for js in range(0,4):
        for is_ in range(0,4):
            ...
            for ly in range(max(1,js+1), min(4,js+4)+1):
                ...
                s1y += s11[ly-1,(ly-js)-1]
                s2y += s12[ly-1,(ly-js)-1]
            aint[is_, js] = (wx*(s1x*s0y) + wy*(s0x*s1y) + wkx*(s2x*s0y) + wxy*(s1x*s1y) + wyy*(s0x*s2y))
    return a, aint, indp, b


def _sor_solve(mx, my, aint, a, indp, b, cij, nit=120, OMG=1.2):
    nx, ny = mx + 3, my + 3
    def Cget(i, j): return cij[(j - 1) * nx + (i - 1)]
    def Cset(i, j, val): cij[(j - 1) * nx + (i - 1)] = val
    def AD(ib, jb, i, j): return a[_AD_offset(ib, jb, i, j, nx)]
        ...
        for ip in range(1, nx + 1):
            for jp in range(1, ny + 1):
                if (indp[(jp-1)*nx+(ip-1)] != 1) and (4 <= ip <= mx) and (4 <= jp <= my):
                    ...
                    for i in range(1,4):
                        for j in range(1,4):
                            ac += aint[i,j]*(Cget(ip+i,jp+j) + Cget(ip+i,jp-j) + Cget(ip-i,jp+j) + Cget(ip-i,jp-j))
                    gs = (b[(jp-1)*nx + (ip-1)] - ac) / aint[0,0]
                else:
                    ac = 0.0
                    i2m = min(ip-1,3); i2p = min(nx-ip,3)
                    j2m = min(jp-1,3); j2p = min(ny-jp,3)
...
def fit_bsplines2d(xd, yd, fd, sd, xmin=0.0, ymin=0.0, xmax=1.0, ymax=1.0,
                   mx0=8, my0=8, n2=2, unl=0.1, und=0.1, rou=1.0, tau=0.5,
                   nit=200, omg=1.2, use_scipy=True, seed=None):
    # fd = np.log(fd)
    A_band, b, KA, NEQ = _assemble_coarse(xmin,ymin,xmax,ymax, mx0,my0, xd,yd,fd,sd, unl,und, rou/(10.0**n2), tau)
    sol = None
    if use_scipy and _HAVE_SCIPY:
        ab = np.zeros((KA, NEQ))
        def Aget(r, c): return A_band[(c - 1) * KA + (r - 1)]
    ...
    kc = mx0 + 3
    cij = np.zeros((my0+3)*(mx0+3))
    for jy in range(my0+3, 0, -1):
        for ix in range(mx0+3, 0, -1):
            cij[(jy - 1) * kc + (ix - 1)] = sol[(jy - 1) * (mx0 + 3) + (ix - 1)]
    for nn in range(1, int(n2)+1):
        mx = mx0*(2**nn); my = my0*(2**nn)
        cij = _prolongate(mx, my, cij)
        a, aint, indp, b2 = _assemble_fine(xmin,ymin,xmax,ymax, mx,my, xd,yd, fd,sd, unl,und, rou/(10.0**(n2-nn)), tau)
        _sor_solve(mx,my, aint, a, indp, b2, cij, nit=nit, OMG=omg)
    return cij, mx, my


def eval_cubic_bspline_surface(x, y, xmin, ymin, xmax, ymax, mx, my, cij):
    # xmin, xmax = xd.min(), xd.max()
    # ymin, ymax = yd.min(), yd.max()
    nx = mx + 3
    hx = (xmax - xmin) / mx; hy = (ymax - ymin) / my
    x = np.minimum(np.maximum(x, xmin), np.nextafter(xmax, xmin))
    y = np.minimum(np.maximum(y, ymin), np.nextafter(ymax, ymin))
    ip = int(np.floor((x - xmin) / hx)) + 1
    jp = int(np.floor((y - ymin) / hy)) + 1
    ip = 1 if ip < 1 else (mx if ip > mx else ip)
    jp = 1 if jp < 1 else (my if jp > my else jp)
    ...
    rx = 0.0 if rx < 0.0 else (1.0 if rx > 1.0 else rx)
    ry = 0.0 if ry < 0.0 else (1.0 if ry > 1.0 else ry)
    bx = np.zeros(4); by = np.zeros(4); BSP3(float(rx), float(hx), 0, bx); BSP3(float(ry), float(hy), 0, by)
    s = 0.0
    for j1 in range(0,4):
        for i1 in range(0,4):
            s += bx[3-i1] * by[3-j1] * cij[(jp + j1 - 1) * nx + (ip + i1 - 1)]
    if s < 0:
        s = 0.001
    return s


def eval_surface_grid(mx,my,cij, nx=100, ny=100, xmin=0.0, ymin=0.0, xmax=1.0, ymax=1.0):
    xs = np.linspace(xmin,xmax,nx,endpoint=False); ys=np.linspace(ymin,ymax,ny,endpoint=False)
    Z=np.zeros((ny,nx))
    for j,y in enumerate(ys):
        for i,x in enumerate(xs):
            Z[j,i] = np.exp(eval_cubic_bspline_surface(x,y,xmin,ymin,xmax,ymax,mx,my,cij))
    return Z


def fit(xd, yd, fd, sd, rou, tau):
    fd = np.log(fd)
    unl, und, mx0, my0, n2 = domain_divisions(xd, yd, fd)
    cij, mx, my = fit_bsplines2d(xd, yd, fd, sd, xd.min(), yd.min(), xd.max(),
                                 yd.max(), mx0, my0, n2, unl, und, rou, tau)
    return cij, mx, my